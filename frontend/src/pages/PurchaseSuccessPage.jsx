...
const PurchaseSuccessPage = () => {
    ...
    useEffect(() => {
        const handleCheckoutSuccess = async (sessionId) => {
            try {
                ...
                if (response.status !== 200) {
                    throw new Error("Failed to confirm payment. Server responded with an error.");
                }
                ...
                setError(error.response?.data?.message || error.message || "An error occurred during checkout success");
            } finally {
        ...
        // If session ID exists, proceed to handle checkout
        if (sessionId) {
    ...
    // If processing is still happening, show a loading message
